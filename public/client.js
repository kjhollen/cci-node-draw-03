...
function setup() {
  ...
  // buffer (image) where we'll draw strokes sent from
  // the server, so that we can repeatedly render this in
  // draw() and layer local feedback/graphics on top of it.
...
function onDrawStroke(data) {
  ...
  // draw all the points in the shape, using begin/endShape + vertex
